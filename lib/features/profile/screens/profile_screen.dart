...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _saveProfile() async {
    ...
    final session = Provider.of<SessionProvider>(context, listen: false);
    final error = await session.updateProfile(name: name, email: email);
    if (!mounted) return;
  ...
  Future<void> _changePassword() async {
    ...
    final session = Provider.of<SessionProvider>(context, listen: false);
    final error = await session.changePassword(
  ...
  Widget _buildAvatarHeader(BuildContext context, SessionProvider session) {
    return Row(
      children: [
        Container(
          width: 64, height: 64,
