...
class _HistoryScreenState extends State<HistoryScreen> {
  final List<GenerationHistory> _items = [];
  bool _isLoading = false;
  bool _hasMore = true;
  int _currentPage = 0;
  String? _errorMessage;

  // Video player para previsualización inline
  ...
  Future<void> _loadPage(int page) async {
    ...
    try {
      final token =
          Provider.of<SessionProvider>(context, listen: false).sessionToken;
      final result =
          await HistoryService.getHistory(token: token, page: page, size: 20);
      ...
      setState(() {
        ...
        _hasMore = result.currentPage + 1 < result.totalPages;
        _isLoading = false;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _delete(GenerationHistory item) async {
    ...
    try {
      final token =
          Provider.of<SessionProvider>(context, listen: false).sessionToken;
      await HistoryService.deleteHistory(token: token, historyId: item.id);
      if (!mounted) return;
      setState(() => _items.removeWhere((i) => i.id == item.id));
      _showSnack('Eliminado del historial', AppColors.success);
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _shareVideo(GenerationHistory item) async {
    ...
    final url = HistoryService.buildVideoUrl(item.mergedVideoUrl!);
      ...
      child: ClipRRect(
        ...
        child: Column(
          ...
          children: [
            ...
            // Video player (si está reproduciendo)
